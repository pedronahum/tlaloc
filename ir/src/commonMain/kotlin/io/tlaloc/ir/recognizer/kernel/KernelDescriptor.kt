...
/**
 * Describes a vendor-fused kernel chosen for a
 * recognized pattern on a specific [KernelTarget].
 *
 * The descriptor is the bridge between pattern recognition and
 * downstream lowering. After [lowerKernelChoice], each `OpKind.COARSENED` op may
 ...
 * @property scratchResults Extra f32 result shapes appended
 *   to the emitted `custom_call`'s results after the op's own. Used by
 *   multi-stage launch chains (`KptxKernelRegistry`): the
 ...
 * @property typedFfi When true, StableHLO emit
