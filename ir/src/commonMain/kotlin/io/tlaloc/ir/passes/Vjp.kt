...
object VjpRegistry {
     ...
     *
     * Rank-2 only: batched MATMUL (rank > 2 + batching/contracting attrs) would need a
     * rank-aware TRANSPOSE + batched MATMUL in [DxirInterpreter] and is deferred.
     *
     * Declared before [rules] because Kotlin initialises `object` properties in source
     * order; a forward reference from `rules` to a later val fails to compile
    ...
    val MatmulRule: VjpRule = object : VjpRule {
        ...
        override fun apply(op: DxirOp, upstream: DxirNode, builder: DxirBuilder): List<Pair<DxirNode, DxirNode>> {
            ...
            require(a.type.rank == 2 && b.type.rank == 2) {
                "MatmulRule: rank-2 operands required, got ${a.type.dims} x ${b.type.dims}"
            }
            val dtype = upstream.type.dtype
            val m = a.type.dims[0]
            val k = a.type.dims[1]
            val n = b.type.dims[1]
            val transposeAttrs = mapOf("permutation" to listOf(1, 0))
            val aT = builder.op(
                OpKind.TRANSPOSE,
                listOf(a),
                DxirType(dtype, listOf(k, m)),
                attrs = transposeAttrs,
            )
            val bT = builder.op(
                OpKind.TRANSPOSE,
                listOf(b),
                DxirType(dtype, listOf(n, k)),
                attrs = transposeAttrs,
            )
            val dA = builder.op(OpKind.MATMUL, listOf(upstream, bT), DxirType(dtype, listOf(m, k)))
            val dB = builder.op(OpKind.MATMUL, listOf(aT, upstream), DxirType(dtype, listOf(k, n)))
