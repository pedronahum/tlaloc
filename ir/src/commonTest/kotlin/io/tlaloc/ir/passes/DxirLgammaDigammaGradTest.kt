 ...
 * adjoint emits — is pinned as REFUSING both transforms loudly (its own
 * derivative is polygamma(2), out of C1's scope).
 *
...
class DxirLgammaDigammaGradTest {
    ...
    @Test
    fun trigammaRefusesBothTransformsLoudly() {
        // TRIGAMMA is gradient machinery: DIGAMMA's adjoint emits it, but its
        // OWN derivative is polygamma(2) — out of C1's scope. Both transforms
        // must refuse by name rather than silently mis-differentiate (the
        // §0.4.392/§0.4.401 refusal-pinning precedent). Second-order reverse
        // through DIGAMMA hits exactly this wall.
        val fn = DxirBuilder.function("trigamma_sum") {
            val x = param("x", r1)
            val tg = op(OpKind.TRIGAMMA, listOf(x), r1)
            listOf(op(OpKind.SUM, listOf(tg), scalar))
        }
        val reverse = assertFailsWith<IllegalStateException> { DxirReverseTransform.apply(fn) }
        assertTrue(
            "no VJP rule registered for TRIGAMMA" in (reverse.message ?: ""),
            "reverse refusal must name TRIGAMMA: ${reverse.message}",
        )
        val forward = assertFailsWith<IllegalStateException> { DxirForwardTransform.apply(fn) }
        assertTrue(
            "no tangent rule for TRIGAMMA" in (forward.message ?: ""),
            "forward refusal must name TRIGAMMA: ${forward.message}",
        )
    }
}
