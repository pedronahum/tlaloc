...
| KV pages sized to the window for sliding layers | ⬜ | A sliding layer's pool keeps the whole history and the mask hides what is outside the window: correct, and wasteful past the window (2048 positions for Muse Glimmer). Freeing or reusing pages behind the window is not written |
