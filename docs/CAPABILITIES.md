...
The suite has **2,672** automated tests: 2,550 that `./gradlew test` runs, and 122
...
of the 2,550 were counted in a clean-room `./gradlew test --rerun-tasks` with 0
failures; the other 14 (Qwen3 and tied-head checkpoint tests) were added later and
ran with the rest of `:ir:jvmTest`, 0 failures.
...
| HuggingFace model families | ✅ | `HfModelFamily` reads Llama and Qwen3 checkpoints through one config parser, name mapping and graph builder, with one `DecoderLayerSpec` per layer. A `config.json` key the family does not know is refused by name at parse; Qwen3's per-head q/k RMSNorm is certified against an independent Kotlin forward pass and, with its negative control, against the real checkpoint |
| Gemma-style layers: sliding window, layers without RoPE, post-attention and post-feedforward norms, logit soft-capping | ⬜ | The config and `DecoderLayerSpec` record them (Qwen3's `use_sliding_window`, `max_window_layers` and `layer_types` are read) and the graph builder refuses each by name; no graph code for them exists |
