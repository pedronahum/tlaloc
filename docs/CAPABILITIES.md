...
The suite has **2,748** automated tests: 2,626 that `./gradlew test` runs, and 122
...
2,626 were counted in a clean-room `./gradlew test --rerun-tasks` with 0 failures,
...
| KV pages sized to the window for sliding layers | ✅ | A model's sliding-window layers keep their KV in a second pool class, the windowed KV pool (`WindowedKvPool`; a `tlaloc-serving-v3` artifact): each sequence holds a ring of at most `ceil(window / blockSize) + 1` pages, block `b` on ring page `b % ringPages`, with its own block table and slot mapping, and a request is split into calls the ring can hold. The full-attention layers keep full-history pages. Certified in the reference interpreter (`WindowedKvPoolTest`: two sequences several windows long through prefill chunks, batched and padded decode steps give logits bit-identical to full-history pools; a ring one page short, or a call one token longer than the ring holds, changes them) and through Triton (`verify.sh`: a three-layer model with window 8 and pages of 4 holds at most 3 windowed pages while its full pages grow to 15, and matches the full-history model sent the same calls to 5.3e-7 of the largest logit). For Muse Glimmer the computed KV per sequence is 365 MiB instead of 832 MiB at 8,192 positions and 989 MiB instead of 3,328 MiB at 32,768 (`docs/SERVING_ARCHITECTURE.md`); Muse Glimmer served from a v3 artifact (its 39 sliding layers in the windowed pool, rings of 160 pages) greedy-decodes the same 32 ids as before, and its pools for four sequences of 32,768 positions take 4,109 MiB on the device against 13,312 MiB with full-history pages (`verify.sh` with `MUSE_GLIMMER=1`). The framework-free Python runtime and the vLLM plugin refuse a v3 artifact by its schema version |
| Long contexts: prefill in chunks, Muse Glimmer up to 32,768 positions and four sequences | ✅ | A prefill entry can take fewer tokens than its context (`-PprefillChunk`); a longer prompt runs as several calls, and the windowed ring is sized for a chunk past the window. The prefill graph gathers each sequence's pages once instead of once per token (`PAGED_ATTENTION` accepts one block table per group of rows): on the GB10 the new lowering gives the per-row lowering's output exactly. In the reference interpreter chunked prefill gives whole-context logits bit for bit, with full-history and windowed pools, and chunks through a ring one page short change them (`BatchedPrefillTest`); through Triton a windowed model with 6-token chunks matches the full-history model sent the same calls (`verify.sh`). Muse Glimmer exported for contexts 512 to 32,768, decode batches 1, 2 and 4 and 512-token prefill calls, served by Triton on the GB10 (`verify.sh` with `MUSE_GLIMMER=1`): a 2,305-token prompt whose fact lies 2,190 tokens before the question, outside the sliding window, gives the 16 ids transformers gives with the same arithmetic (its answer starts with the fact, " 4719."; logits within 8.4e-3 of the largest, 2e-2 required), and the two short prompts keep their 32 ids. Measured with `triton/context_bench.py`: a decode step of one sequence takes 265 ms at context 512 and 317 ms at 32,768; four sequences together make 16.7 tokens/s at 512 and 9.6 at 32,768; prefill runs at 766 tokens/s for 432 tokens and 142 tokens/s for 32,688 (230 s); the machine peaked at 87 to 90 GiB in use (table in `docs/SERVING_ARCHITECTURE.md`). |
| Long-context attention that reads only what it needs | ⬜ | Today every decode step and prefill call gathers and scores every position of its context bucket, the sliding layers' too (masked outside the window): at 32,768 positions that is why a 512-token prefill call takes about 4.5 s against 0.55 s at 512. Scoring only a sliding layer's window, and a paged kernel that does not gather, are not started |
| bf16 weights in a serving artifact | ✅ | `HfDecoderConfig.weightDType` BF16 (Muse Glimmer's default; `-PweightDType=bf16` for Llama and Qwen3) stages the weights as the file's bf16 bytes, copied a block at a time (the Linears transposed in bands, so the 2.7 GB embedding table and head never sit in one array); each projection rounds its f32 input to bf16 and multiplies into f32. The streaming writer is checked bit for bit against the in-memory staging with a 64-byte block. The Triton backend drops the page cache of each weight file after uploading it. Qwen3-0.6B with bf16 weights through Triton (`verify.sh`): 1136 MiB on the device against 2273 MiB, all 32 fixture ids equal HuggingFace's, logits within 3.0e-3 of the largest (6e-3 required; the f32 tolerance of 2e-3 fails, so the comparison tells the two apart), 11.3 ms a decode step against 15.4 ms |
| A tied head without a second copy of the embedding table | ✅ | A tied head is one `MATMUL` that contracts the hidden axis of the final state and of the table (`dot_general`, `contracting_dims = [1] x [1]`, no transpose). In the interpreter it gives the logits of a transposed copy bit for bit (f32 and bf16 weights; a changed copy element is seen). Qwen3-0.6B through Triton: 310 weights, 2273 MiB against 2867 MiB, the same 32 ids, logits within 5.6e-4 |
| Placeholder token ids refused when serving | ✅ | The manifest lists a multimodal checkpoint's image and video placeholder ids (`model.refusedTokens`); the Triton backend refuses a request holding one by name and leaves the sequence as it was, and so does `tlaloc_serve.py`. `verify.sh`: stand-in ids of the window models over HTTP and gRPC (the next logits equal a sequence that never sent the request; an unlisted id is accepted), and Muse Glimmer's 200091 and 200092 with `MUSE_GLIMMER=1` |
| A full KV page pool (Triton) | ✅ | Pages are reclaimed only from sequences idle past the reclaim rule (Triton has ended them), least recently active first and only as many as needed; otherwise the request is refused by name, listing the sequences that hold pages, and a sequence refused mid-generation keeps its KV. `verify.sh` (TinyLlama): the refused sequence, sent the same request after another ends, continues with its solo ids; the reclaim order and count are read from the server log; a sequence whose requests are refused still counts as active and is not reclaimed, and a refused second START frees the earlier sequence's KV (both checked, and both failed before the fix) |
| Preempting a live sequence (KV swap or recompute) | 📐 | Not built: a live sequence is never preempted |
...
| vLLM platform plugin | ✅ | vLLM 0.29.0's `LLM.generate()` runs a real TinyLlama from a Tlaloc artifact — the same 6 token ids as the direct driver and as HuggingFace; a new sequence's prompt is written by one prefill call when the artifact has an entry that holds it (the direct driver's use of it is asserted; the runner's is unit-tested). One sequence, greedy, prompt within the compiled context; `vllm serve`'s HTTP layer is not yet run ([audit](INFERENCE_SERVING_AUDIT.md), [runbook](SERVING_RUNBOOK.md)) |
| NVIDIA Triton backend (`libtriton_tlaloc.so`) | ✅ | Triton 25.11 on the GB10 compiles Tlaloc StableHLO through the PJRT C API and serves it over HTTP and gRPC. `triton/verify.sh` certifies: a reverse-mode gradient bit-identical to the DXIR interpreter; FP32, FP64, FP16, BF16, INT8, INT32, INT64, UINT8 and BOOL over HTTP and gRPC (FP16, INT8 and UINT8 from a hand-written module, since Tlaloc has no such dtypes); shape buckets; dynamic batching of the gradient model (512 concurrent one-row requests, each with exactly its own row, in about a third as many executions; about 5x the unbatched throughput); ragged batches of two widths grouped by the shape of their rows (216 requests in 74 to 76 executions against 119 to 134 when only consecutive requests of one width run together, the same rows); CUDA shared-memory inputs read in place through a PJRT view and outputs copied device to device, with the same bits as the host path (16 MiB each way: about 0.5 ms of server time against about 4.5 ms through the host); decode steps with backend-held KV pools; sequence mode with prefill and batched decode; load-time refusals by name; TinyLlama-1.1B greedy decoding with the same 6 token ids as HuggingFace; Qwen3-0.6B with the same 16 ids as HuggingFace for a plain and a chat-template prompt; and, opt-in (`MUSE_GLIMMER=1`), Muse Glimmer's text decoder with bf16 weights (row above) ([README](../triton/README.md)) |
...
| Prompts of several sequences prefilled in one call (Triton) | ✅ | Prefill entries for every batch of the ladder (`HfServingExport.specs`; `-PprefillMaxBatch` caps them). Each sequence is a right-aligned row with its own positions, block table and slots; padding writes nowhere. In the reference interpreter (`BatchedPrefillTest`) four prompts prefilled together, three in a batch-4 entry with a padding row, and second chunks at different positions give their solo logits and continuations bit for bit, with full-history and windowed pools; padding that writes its KV, or a left-aligned row, changes them. The Triton backend runs the prompts of one batch that share a context bucket as one call. On the GB10 (`verify.sh`), 2 and 4 TinyLlama and Qwen3-0.6B prompts sent together ran in one execution and gave their solo argmax and the same 8 greedy ids, logits within 9.0e-4 of the largest (TF32; the bound is 5e-3). Four TinyLlama prompts prefill in about 35 ms against about 110 ms one after the other, and four Qwen3-0.6B prompts in about 61 ms against about 100 ms. Prompts whose request also carries END were not batched together by Triton's sequence batcher |
