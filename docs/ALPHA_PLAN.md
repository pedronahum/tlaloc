...
| **Signing** | ✅ | Central validated every `.asc` of the 0.1.0-alpha01 upload (deployment `31584165-2f58-4a0f-bca2-66503bd028d5`, state VALIDATED, no errors) | In-memory key from `signingInMemoryKey` / `SIGNING_IN_MEMORY_KEY`; key `0A5C1EB754B5381D`, published to keys.openpgp.org and keyserver.ubuntu.com. |
| **Central publishing repository** | ✅ | `./gradlew releaseToCentralPortal --no-parallel` uploaded all 24 publications and `centralPortalHandoff` got HTTP 200; the Portal listed the deployment `31584165-2f58-4a0f-bca2-66503bd028d5` as VALIDATED | Released 2026-09-24. |
| **`io.github.pedronahum` namespace on Central** | ✅ | Verified in the Central Portal; `0.1.0-alpha01` is served from `repo1.maven.org/maven2/io/github/pedronahum/` | The plugin marker in the sub-group `io.github.pedronahum.tlaloc` landed in the same deployment and is live. |
...
| **Release procedure** | ✅ | [RELEASING.md](RELEASING.md), run end to end for 0.1.0-alpha01 on 2026-09-24 | Publish was pressed in the Portal UI (Deployments → the VALIDATED deployment → Publish). The Portal's "Publish Component" page is a separate bundle-upload form and is not part of this procedure. |
...
| **An actual Maven Central release** | ✅ | `0.1.0-alpha01` published 2026-09-24; `TLALOC_SMOKE_FROM_CENTRAL=1 bash scripts/onboarding-smoke.sh` builds and runs the README and GETTING_STARTED install blocks against Central with no `mavenLocal()` | https://repo1.maven.org/maven2/io/github/pedronahum/ |
...
| R8 | No tag-triggered release workflow | P1 | B | 🧪 `.github/workflows/release.yml`: `v*` tags + workflow_dispatch, refuses a non-tag ref or tag ≠ `v<version>` (script exercised locally for match, mismatch, branch), refuses a version whose `tlaloc-core` POM is already on `repo1.maven.org` (the curl check returns 404 for `0.1.0-alpha01` and 200 for a published POM), `./gradlew test`, `scripts/onboarding-smoke.sh`, then `releaseToCentralPortal --no-parallel` with the four named secrets; YAML parsed; not yet run on GitHub. Critic pass: an armoured key folded to one line with literal `\n` (what `setup-signing-key.sh` writes) failed signing when passed as `SIGNING_IN_MEMORY_KEY` ("Could not read PGP secret key"); the build now unfolds it, and `:bom:signBomPublication` signs from the environment with the folded and the multi-line form (the folded form fails with the unfolding removed) |
...
| R10 | No Gradle plugin: consumers hand-wire `kotlinCompilerPluginClasspath` per source set and pass options as raw `-P` args; nothing aligns plugin and library versions (no BOM) | P1 | C | ✅ `:gradle-plugin` (`tlaloc-gradle-plugin`, id `io.github.pedronahum.tlaloc`, Java 21 bytecode, plus its unprefixed marker under group `io.github.pedronahum.tlaloc`) applies `tlaloc-compiler-plugin` of its own build-time version to every Kotlin/JVM compilation; `tlaloc { }` maps all five compiler-plugin options; JS/Native/Wasm/Android compilations skipped with a warning naming them, no Kotlin plugin refused by name. `:bom` (`tlaloc-bom`) constrains all 22 published artifacts, and `verifyBomCoverage` (in `check`) fails when a published artifact is missing from it (negative-tested). Both pass `verifyPomMetadata` and sign. `scripts/onboarding-smoke.sh` builds four consumers from mavenLocal: plugin + every option runs `grad {}` and writes `dumpGradSourceDir`; without the plugin the same program fails with the plugin-missing error; a JS target is skipped by name; no Kotlin plugin refuses. Each check was seen failing against a broken plugin (misspelled option, JS applied, refusal removed). `quickstart` and `differentiable-physics` use the plugin + BOM; `readable-gradients` keeps the manual route, now on `kotlinCompilerPluginClasspathMain` (plain `kotlinCompilerPluginClasspath` had put the plugin on its `printed` compilation too). Review: a Tlaloc plugin declared in a root `plugins { }` block with the Kotlin plugin declared only in a subproject died with a bare `NoClassDefFoundError`; it is now refused by name, and the smoke's fifth consumer checks both the recommended multi-project layout (runs) and that one (refused; seen failing with the catch removed). The no-Kotlin refusal no longer says "no Kotlin Gradle plugin" to a `kotlin("android")` project. A consumer on Gradle 8.14.3 with its daemon on JDK 21 applies the plugin, reads `tlaloc { }` and runs `grad { }` (checked by hand, not in the smoke). Critic pass: the options mapping and refusals were pinned only by the smoke, which no workflow ran. `:gradle-plugin:test` (`TlalocGradlePluginTest`, 6 ProjectBuilder tests with the real Kotlin Gradle plugin) now checks the defaults and every option on `compileKotlin`/`compileTestKotlin`, the compiler-plugin artifact at this version on `kotlinCompilerPluginClasspathMain`, the JS/metadata skip, the no-Kotlin refusal and the compiler plugin id; each fails under its negative control (misspelled option, JS applicable, refusal removed, `PLUGIN_ID` changed). The compiler plugin id is read at build time from `TlalocCommandLineProcessor.PLUGIN_ID` instead of a second literal. `release.yml` runs the smoke before uploading. After alpha01: `dumpGradSourceDir` reached the compile task as an absolute-path input, so a compilation that set it did not relocate in the build cache, and the dumped files were not an output, so a cache hit did not restore them; declaring the shared directory an output of both `main` and `test` was refused by Gradle. Each compilation now writes into `<dumpGradSourceDir>/<default source set>` (`main`, `test`; `jvmMain`, `jvmTest` in Multiplatform), passed as a KGP `FilesSubpluginOption` (`FilesOptionKind.INTERNAL`, which `CompilerPluginConfig.getAsTaskInputArgs` leaves out of the task inputs) and registered as an optional output (`tlalocGradSourceDir`) of that compilation's compile task. No default directory: `dumpGradSource` alone still only prints. `DumpGradSourceBuildCacheTest` (TestKit, consumers resolving Tlaloc from a build-local `functionalTest` repository the `:gradle-plugin:test` task publishes to first) checks that `main` and `test` write to separate subdirectories, that after deleting `build/` both compile tasks are FROM-CACHE with the same dumped files, and that a copy of the project at another absolute path is FROM-CACHE for both; its second test is the in-suite negative control (the raw `-P` option with an absolute path is not FROM-CACHE after relocation). Negative controls by hand: the plain `SubpluginOption` fails the relocation assertion, no output registration fails the restore assertion, no per-compilation subdirectory fails the separation assertion. `TlalocGradlePluginTest` now also checks the per-compilation path, the files option, the registered output, that the path is absent from the task input args, that no output is added without a directory, and that a Multiplatform `jvm()` target writes into `jvmMain` and `jvmTest` (fails when the subdirectory is named after the compilation instead). The onboarding smoke and `examples/differentiable-physics` read `build/gradients/main` |
...
| R17 | Internal section numbers and jargon inside user-visible exception and diagnostic strings (~29 literals) | P1 | E | ✅ No `§` left in any string literal of main code (26 literals) and the jargon beside them ("named deferral", phase ids, "landmine", "honest", "twin gap") rewritten as plain statements in core, ir, autograd, stablehlo, kptx, maestro, runtime-pjrt, compiler-plugin and the Python serving runtime; the strictLowering and Kotlin-version hints name the `tlaloc { }` spelling; `grad`/`vjp`/`jvp` KDoc cites no §. Tests updated and seen failing against the old strings: `SafetensorsTest.fp8IsRefusedByName`, `PjrtTpuLocalCertTest.cudaSessionRefusesNullOptionsByName`, `DemotedKindRefusalTest` (3). Review pass: phase ids left in messages of `Tape`, `KvCacheWriteAttrs`, `DxirForwardTransform`, nn `Dense`/`BatchNorm` (G1d, H5, B5, F2, F4) and "named deferral" in the vLLM plugin's refusals rewritten; `vllm_tlaloc_test` and the live `VllmLivePluginTest` (ran against vLLM 0.29.0) pinned to the new text and seen failing against the old. Internal-invariant messages in `PhiCalculus` keep their F1/F2 labels |
| R18 | README: `contract(a, b)` does not compile (it is infix); the training snippet uses example-local helpers; test counts disagree across docs | P0 | E | ✅ `activations contract weights`; the training snippet uses `capture` + `CapturedStep.run` + `Adam` + `saveCheckpoint`; both snippets are mirrored verbatim in `nn` `ReadmeSnippetsTest` (trains, checkpoints, reloads bit-identically; review pass made the training snippet byte-identical to the README, it had an extra line; the old `contract(a, b)` spelling fails to compile there). README states no test count; CAPABILITIES is the one place, with a placeholder for the stage-G recount. Examples table lists `named-indices`; copyright line; install via Gradle plugin + BOM. Critic pass: the install block did not compile (`Inconsistent JVM-target compatibility … 'compileJava' (25) and 'compileKotlin' (21)`); it now has the `JvmTarget` import and a `java { }` block, and labels `tlaloc-runtime-pjrt` as JDK 25 at run time. `onboarding-smoke.sh` check 6 builds the README's settings and build blocks verbatim and runs `grad { }` (seen failing on the block without `java { }`) |
| R19 | GETTING_STARTED: no GPU setup, no configuration table (9 env vars/properties undocumented), no troubleshooting, deprecated `kotlinOptions`, dead "Requirements" link, stale shape-error instructions | P1 | E | ✅ Rewritten: real clone URL, settings + build files with `jvmToolchain(25)`/`JVM_21`, `tlaloc { }` and `compilerOptions.freeCompilerArgs` spellings, `./gradlew -p examples/quickstart shapeError`, new "Running on a GPU" (JAX plugin install, search order, native access, memory), "Configuration" (every `getenv`/`getProperty` in main code, defaults read from source) and "Troubleshooting" sections; no § numbers. Critic pass: the section 2 build block lacked the `java { }` block section 0 prescribes and did not compile; fixed, and `onboarding-smoke.sh` check 6 builds its settings and build blocks verbatim |
| R20 | Section numbers, `/home/pedro` paths and self-narrating prose across user-facing docs; CAPABILITIES contradicts itself about CI; RELEASING names the old `~/.m2` path | P1 | E | ✅ README, CHANGELOG, GETTING_STARTED, COMPATIBILITY, CAPABILITIES (CI lanes now ✅ green on GitHub Actions, kotlin-next marked as a probe), READABLE_REVERSE, RELEASING, SERVING_RUNBOOK (commands repo-relative, the stale "LLM.generate uncertified" block replaced) and every example README carry no §0.4 numbers and no `/home/pedro` paths; "honest" headings are "Limitations". CHANGELOG rewritten as one 0.1.0-alpha01 entry incl. group id, rename, Gradle plugin/BOM and the review's fixes. `scripts/check-doc-links.py` (committed in the critic pass; it was a scratch script before) finds every relative link and heading anchor in the 26 user-facing Markdown files resolving and every code fence balanced, and fails on a broken link. Review pass: SERVING_RUNBOOK's remaining narration ("this file used to", "slice", "landmine") and three internals READMEs' rewritten; CAPABILITIES' kotlin-next row no longer claims the lane showed the library passing (the API shows only step conclusions, which are green by construction; the library result is the local 2.4.20 run). Critic pass: CAPABILITIES' CI rows and the README said the lanes are green, but only `d0ca85b` has run (build: success, read from the Actions API); the rows now name that commit and RELEASING §3 requires the tagged commit green on every `build.yml` lane. § numbers removed from the root build's error and log messages, `docs/vendoring.md` and `harness/python/README.md`; CHANGELOG says the Maestro step type is unpublished and records the Python `PjrtApi.load` change. RELEASING lists `tlaloc-gradle-plugin`, `tlaloc-bom` and the plugin marker, a check of the 24 components before Publish, the release-day doc edits, and when to push a tag after a local release |
| R21 | Vendored Netflix Maestro: no root NOTICE, modified files not marked as modified, Tlaloc-written files carry a Netflix copyright; ALPHA_PLAN claims no Apache source is redistributed | P1 | F | ✅ Root `NOTICE` credits Netflix Maestro at the pin (upstream ships none at `0150f2a7`: its GitHub tarball at the pin has no NOTICE, and diffing that tarball against `third-party/maestro` shows exactly the five changed files below plus Tlaloc's added files); the five changed upstream files (`build.gradle`, `settings.gradle`, `StepType`, `KubernetesCommand`, `MaestroStepRuntimeConfiguration`) carry a `Modified by` line and are listed in `docs/vendoring.md`; the 14 Tlaloc-written Java files carry `Copyright 2026 Pedro N. Rodriguez`; the Tier 0 LICENSE row no longer says no Apache source is redistributed. `VendoringNoticeTest` in `:vendored-maestro:maestro-tlaloc` (3 tests; each fails under its negative control; review pass also failed it by unmarking `KubernetesCommand` and by marking an unlisted `maestro-http/build.gradle`). The test checks the marks against the table, not the tree against upstream: an unmarked, unlisted edit to an upstream file passes it. Found, not fixed: `maestro-server:compileJava` fails at HEAD because it never declared its `maestro-tlaloc` dependency, and `spotlessJava`/`checkstyleMain` in the vendored build crash on JDK 25 before reading any file; owned by R24 |
| R22 | No CONTRIBUTING, SECURITY, issue/PR templates or dependabot; README names no copyright holder; `docs/papers` text lacks its CC-BY attribution note | P1 | F | ✅ `CONTRIBUTING.md`, `SECURITY.md` (GitHub private vulnerability reporting), bug/feature issue templates + `config.yml`, PR template, `dependabot.yml` (gradle with `third-party/**` excluded, github-actions), `docs/papers/README.md` (DOI, CC-BY-4.0, the extraction named as the change), README License names the holder, NOTICE and the paper licence, README links CONTRIBUTING/SECURITY, `.gitignore` adds `*.hprof` (`hs_err_pid*` was already there). Link checker clean over the new files |
| R23 | Clean-room re-certification of everything above, then an adversarial critic pass over the whole diff | — | G | Part 1 ✅: cleared `~/.m2/repository/io/github/pedronahum`, `publishToMavenLocal` wrote 23 `tlaloc-*` artifacts (22 constrained by the BOM, plus the BOM) + the plugin marker, every POM with name/description/url/licence/developer/scm, `.asc` beside every file (signature verified with gpg), no Symja, `api` deps as `compile` scope, compiler-plugin's core/ir as `runtime`; `./gradlew test --rerun-tasks` 2,507 tests, 0 failures, 93 skipped by name (88 MLIR round trips without `stablehlo-translate`/`sdy-opt`, 5 TPU); maestro lanes 50 + 4; combined 2,561 (= the 2,525 baseline + 12 compiler-plugin + 24 runtime and kptx − 5 deleted `MaestroDescriptorTest` + 3 `VendoringNoticeTest` + 2 `ReadmeSnippetsTest`), written into CAPABILITIES; `apiCheck`, `verifyPomMetadata`, `verifyBomCoverage`, `verifyJvmTarget`, `onboarding-smoke.sh` (5 consumers), `jdk21-smoke.sh` green; `releaseToCentralPortal --dry-run` puts 24 sign tasks, every central upload and `centralPortalHandoff` in the graph (nothing sent); all ten examples ran (gpu-training and mnist on CUDA, gpu-inference both halves incl. TinyLlama `' Paris.'` and the reference graph, tpu's device half skipped by name and its `--target cuda` dry run 5/5). Part 2 ✅: critic pass over 3bb86b5..29a7b75 found 15 items; fixed in the critic-fix stage (rows R8, R10, R18, R19, R20 record them; every published jar, classes, sources and javadoc, now carries `META-INF/LICENSE` and `META-INF/NOTICE`; the combined count is 2,567 with the 6 `TlalocGradlePluginTest` tests, run with `--rerun`, not in a fresh `--rerun-tasks` suite); deferred: `dumpGradSourceDir` build-cache relocation (R10, since done), the vendored build (R24, since done), CI on the review's commits (not pushed from this stage) |
| R24 | Vendored Maestro build: `maestro-server:compileJava` fails (no declared `maestro-tlaloc` dependency); `spotlessJava`/`checkstyleMain` crash on JDK 25 | P2 | — | ✅ `maestro-server/build.gradle` declares `maestro-tlaloc`. With the dependency declared, compileJava found a second break: the `tlaloc` bean called a 7-argument `TlalocStepRuntime` constructor that now takes 10; the configuration now also builds a `TlalocPodSpecBuilder` bean and passes `TLALOC_CLUSTER_VENDOR`/`TLALOC_CLUSTER_ARCH` from the environment. The lint failures were not JDK 25 in Checkstyle's case: Gradle 9 defaults to Checkstyle 10.24, which rejects the `scope` property in `config/checkstyle/checkstyle.xml`; the build pins 9.3, Gradle 8.8's default (upstream's wrapper), which runs on JDK 25. Spotless 6.25 resolved google-java-format while configuring the task, which Gradle 9 refuses in a composite build, and google-java-format below 1.30.0 refuses JVM 25; the build uses Spotless 8.10.2 with google-java-format 1.30.0 and excludes the three upstream `maestro-engine` files whose switch-case comment indentation 1.30.0 changes, rather than reformatting them. Tlaloc's own files were brought under the vendored rules: `spotlessApply` on `maestro-tlaloc` and the two Tlaloc-marked `maestro-common` files, 45 Checkstyle errors in `maestro-tlaloc` (braces, repeated literals, magic numbers) and its one remaining PMD violation fixed. `./gradlew -p third-party/maestro build -x test --rerun-tasks` green (165 tasks: every module's compileJava, spotlessCheck, checkstyleMain, pmdMain); `:vendored-maestro:maestro-tlaloc:test` 50, `maestro-server:test` 68, `maestro-common:test` 343, all passing. New `MaestroStepRuntimeConfigurationTlalocTest` (maestro-server) checks the configuration registers the Tlaloc runtime under `TLALOC`; it fails with the registration line removed. `VendoringNoticeTest` failed with `maestro-server/build.gradle` marked but unlisted, and passes with the row added. `maestro-integration.yml` gains a compile-and-lint step and a `maestro-server:test` step, and its paths filter covers `maestro-server/**` and the vendored build files; on GitHub the lane passed every step, the compile-and-lint and `maestro-server:test` steps included, on 48639a6 (run 35981567066) |

Stages run one at a time: builds on this machine never overlap.

## Post-release housekeeping (2026-09-24)

Work on `main` after the `v0.1.0-alpha01` tag. The version stays `0.1.0-alpha01`;
nothing was published. `CHANGELOG.md` records the user-visible changes under
`[Unreleased]`.

- Dependabot: `actions/checkout` 7, `actions/setup-java` 6, `actions/upload-artifact` 7,
  Symja 3.2.0, kotlinx-coroutines 1.11.0 (declared in the catalog, used by no module),
  the Gradle wrapper 9.7.1 and Kotest 6.2.5 are integrated. `gradle/actions` stays on
  v5: v6 moves its caching into a proprietary component under the Gradle Terms of Use,
  and `.github/dependabot.yml` ignores it. All eight pull requests are closed.
- KDoc in every published module no longer cites changelog numbers, plan phases or
  planning documents (1,346 lines); `checkKdocInternalRefs` fails `test` and `check`
  when one comes back.
- The vendored Maestro build compiles and lints on JDK 25 with Gradle 9, and
  `maestro-integration.yml` compiles it and runs `maestro-server:test` (R24).
- `dumpGradSourceDir` relocates in the build cache (R10).
- Suite: 2,517 tests in `./gradlew test --rerun-tasks`, 0 failures, 93 skipped by
  name; 122 more in the vendored Maestro lanes; 2,639 combined. `apiCheck`,
