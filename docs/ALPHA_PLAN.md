...
| R20 | Section numbers, `/home/pedro` paths and self-narrating prose across user-facing docs; CAPABILITIES contradicts itself about CI; RELEASING names the old `~/.m2` path | P1 | E | ✅ README, CHANGELOG, GETTING_STARTED, COMPATIBILITY, CAPABILITIES (CI lanes now ✅ green on GitHub Actions, kotlin-next marked as a probe), READABLE_REVERSE, RELEASING, SERVING_RUNBOOK (commands repo-relative, the stale "LLM.generate uncertified" block replaced) and every example README carry no §0.4 numbers and no `/home/pedro` paths; "honest" headings are "Limitations". CHANGELOG rewritten as one 0.1.0-alpha01 entry incl. group id, rename, Gradle plugin/BOM and the review's fixes. A link checker over these files finds every relative link and heading anchor resolving and every code fence balanced |
