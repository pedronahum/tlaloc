...
| R11 | `PjrtSession.executeOn` shares one `ExecuteContext` per executable across threads while its KDoc promises thread safety; `close()` can race an in-flight call | P1 | D | ✅ per-executable context lock in `executeOn`, read/write lifetime lock so `close()` waits for in-flight calls, buffers refuse use after close; `PjrtSessionConcurrencyTest` (8 threads × 200 `executeOn` on GB10 CUDA; `closeBlocksUntilAnInFlightCallReturns`) — both negative controls fail (SIGSEGV / close returns early) |
