...
| **Plugin ↔ library version match still unchecked** | ⬜ | — | Carried forward unchanged from Tier 0. §0.4.503 added a guard for the *Kotlin compiler's* version; it did NOT add one for `io.github.pedronahum:compiler-plugin` vs `io.github.pedronahum:core`. Mixing those is still unsupported and still silent, as `docs/COMPATIBILITY.md` says. A separate item. |
...
- **The KMP publications carry no `org.gradle.jvm.version`, so Gradle will not catch
  ...
  - BAD: a consumer at target 21 who adds `io.github.pedronahum:runtime-pjrt` gets NO
...
| Item | Status | What pins it | Deferred / notes |
|---|---|---|---|
| **Platform matrix: x86_64 Linux + arm64 macOS** | 🧪 | `.github/workflows/build.yml`, job `test`, `matrix.os = [ubuntu-latest, macos-15]`, `fail-fast: false`. The YAML parses and **`actionlint` 1.7.12 reports zero findings** on all three workflow files (its runner-label, expression and step-id checks included) | **Never run.** macOS earns a lane because the repository has supported it as a dev platform since `scripts/setup-mac-bootstrap.sh` / `setup-mac-userspace.sh` and the root build injects `/opt/homebrew/bin` into every `Test` task's PATH for it. What that lane will NOT do is named in the file: a GitHub macOS runner *has* `/opt/homebrew/bin` and does *not* have `stablehlo-translate` / `sdy-opt` / `iree-compile`, because `setup-mac-userspace.sh` builds them with bazelisk from pinned openxla commits (20–60 min), so the MLIR round-trips self-skip there exactly as on Linux. macOS minutes bill at **10×** on private repositories — a cost note, in the file, for the person paying it. |
