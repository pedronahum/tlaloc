...
| R24 | Vendored Maestro build: `maestro-server:compileJava` fails (no declared `maestro-tlaloc` dependency); `spotlessJava`/`checkstyleMain` crash on JDK 25 | P2 | — | ⬜ deferred: `third-party/maestro` is not published and `:vendored-maestro:maestro-tlaloc` (with `VendoringNoticeTest`) builds; fixing the upstream build's JDK 25 tooling is outside the alpha release |
