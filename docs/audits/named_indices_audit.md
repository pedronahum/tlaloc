...
| StableHLO emitter produces correct `dot_general` contracting/batching dims from named-axis attrs. | Dual paths in [`stablehlo/StablehloEmitter.kt:emitMatmul`](../../stablehlo/src/commonMain/kotlin/io/tlaloc/stablehlo/StablehloEmitter.kt): **explicit** (lines ~770–800) reads `lhs_contracting_dims` / `rhs_contracting_dims` set by N.2; **named-inference** (new, lines ~708–737) derives positions from `axisNames` intersection when explicit attrs absent. |
...
- **OQ-5: Rank-3 / Rank-4 batched named contraction.** **RESOLVED in §0.4.242 (Layer 1.5 cleanup).** Two new overloads land in [`core/ops/NamedOps.kt`](../../core/src/commonMain/kotlin/io/tlaloc/core/ops/NamedOps.kt): `Rank3<B, M, K> contract Rank3<B, K, N> → Rank3<B, M, N>` (one batching axis) and `Rank4<NB, NH, NT, ND> contract Rank4<NB, NH, ND, NT'> → Rank4<NB, NH, NT, NT'>` (two batching axes — the QK^T attention forward shape). The plugin's `emitContract` was generalised from the v1 "exactly one shared name" rule to a position-based partition: shared names at *matching* positions are batching, at *different* positions are contracting. Special-cased for rank-1 × rank-1 (the unique shared axis is always contracting). Tests added: `Rank3 batched contract emits MATMUL with one batching axis`, `Rank4 attention contract emits MATMUL with two batching axes`, plus a rank-4 round-trip pin (`namedAttentionRank4BatchedRoundTrips`) through `stablehlo-translate`. **Closing rationale:** the user flagged this as load-bearing for Layer 2 — leaving a hole in the type system at attention's rank-4 shape would force the typed-step-boundary work to walk around it. Rank-5+ contractions remain a future-work item but are no longer in the critical path.
