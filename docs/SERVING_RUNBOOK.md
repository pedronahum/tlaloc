...
This runbook takes a fresh machine to a Tlaloc serving artifact, runs that
artifact from Python with **no JVM, no JAX, no torch and no numpy in the process
(a PJRT plugin `.so` and a driver are the entire runtime)**, and hands it to vLLM
through the `vllm-tlaloc` platform plugin. Every step is marked **CERTIFIED** (a
test in `./gradlew test` proves it) or **UNCERTIFIED** (written, never executed,
with the reason and the command that would settle it).

**Start at section 10 for the demo.** A real TinyLlama-1.1B (all 22 layers, from
its own HuggingFace checkpoint) greedy-decodes through an exported artifact on
PJRT-CUDA and produces the **same token ids HuggingFace transformers produces**:
`Paris.\n\n2.`, six for six. Section 11 runs the same artifact through vLLM
0.29.0's `LLM.generate()`. Sections 1–4 are the machinery underneath.
...
**The artifact is the deployment.** After step 2 returns, a directory of JSON and
textual StableHLO is the entire input to serving; nothing downstream calls back
into Kotlin, and the same directory is what would serve on TPU
([TPU_BRINGUP.md](TPU_BRINGUP.md)).

Run every command from the repository root. Gradle commands need a JDK 25
...
the CUDA-12 jaxlib plugin, JDK 25.

```bash
export JAVA_HOME=/path/to/jdk-25                  # every gradle command
export TLALOC_VENV=~/.local/venvs/iree            # the development venv
```

What the development venv has, and why. **The serving path needs none of it.**
...
| `torch`, `safetensors` | **no** | oracles only (the bf16 parity test reads bytes torch wrote) |
| a PJRT plugin `.so` | **YES — and it is the only one** | `tlaloc_pjrt` dlopens it and calls `GetPjrtApi` |

### What a SERVING machine actually needs

```bash
python -m venv /srv/tlaloc-venv && . /srv/tlaloc-venv/bin/activate
pip install -e harness/python   # zero dependencies
...
frozen — read section 0.1 before typing `pip` at it again.
...
tightest on (see section 3).

---

## 0.1 `~/.local/venvs/iree` is FROZEN — the oracle venv
...
| `write_llama_safetensors.py` | bit-exact safetensors parity (torch writes the bytes we then claim to read) |
| `tlaloc_serve.py`, `run_tlaloc_serve_check.py` | section 4's "a Python process with no JVM runs the artifact", at 1e-5 CPU / 1e-3 CUDA |
...
A dry run of `pip install vllm` into this venv planned **186 packages**:
`torch 2.13` replacing the oracle torch, **33 CUDA-13 wheels** beside jax's
...
- **The policy is a pure function, and it is tested against a venv that
  is wrong**: the recorded bad inventory (torch 2.13, a `jax-cuda13`
...
```bash
./gradlew test --rerun-tasks 2>&1 | tee /tmp/cert1.log | tail -5
./gradlew \
...
Six entries and their bodies land in that directory. The task is
...
```bash
./gradlew :maestro:jvmTest --rerun-tasks \
...
F16, I64 and fp8 are **refused by name**. A real checkpoint's weights are staged
as a weight table beside the program (section 10).

---

## 3. Run the artifact from Python (CERTIFIED, both lanes)

No JVM, no gradle, no Kotlin and no framework in this process. The directory plus a plugin `.so` is the input.

```bash
. "$TLALOC_VENV/bin/activate"
export PYTHONPATH=$PWD/harness/python
python - <<'PY'
...
Host arrays are **flat row-major Python lists**, not ndarrays:
...
**Which engine ran, and why there are two.**
...
## 3.1 PJRT without a framework — `tlaloc_pjrt.py` (CERTIFIED)
...
with api.create_client(platform="cuda") as client:   # create_options ALWAYS
...
client and hangs the machine. A TPU client, by contrast, gets
**no** options at all — those knobs are the XLA GPU plugin's.
...
`tlaloc_serve.py` runs on this binding, so the serving loader is
framework-free as well.

## 4. Plug it into vLLM (CERTIFIED through the worker API)
...
~/.local/venvs/vllm/bin/python -m pip install -e harness/python
...
### What the live vLLM lane certifies
...
**`LLM.generate()` end to end is certified in section 11.** `vllm serve`'s HTTP
layer has not been run. The command it would take:
...
### Why it is a second venv and not this one
...
pip install -e harness/python   # zero deps
export TLALOC_PJRT_PLUGIN_PATH=/path/to/xla_cuda_plugin.so
```

#### What the vLLM venv will and will not need
...
Tlaloc's account. An earlier recipe said `pip install vllm
...
`.so`, and the way to get one into that venv without installing jax
...
**Answered by the live lane:** vLLM's CUDA-13 torch and the CUDA-12 XLA plugin
...
| `platform.py` — `TlalocPlatform` | **yes** | **yes**, in the vLLM venv |
| `worker.py` — `TlalocWorker` | **yes** | **yes**, in the vLLM venv |
...
at all. The bottom two rows are now
**yes**, and the bug the live lane found was in the TOP half, reached only by driving
...
## 4.1 Fetch a real Llama checkpoint (CERTIFIED as far as ingestion)
...
```bash
./gradlew :ir:jvmTest --tests "*HfLlama*"
...
The decode graph built from those weights is certified against transformers at
1e-5, and the artifact carries them as a staged weight table. **section 10
is the demo**, and it starts by downloading this same checkpoint. What
remains uncertified is `vllm serve`'s HTTP layer (section 4).
...
way — see [KPTX_PAGED_PERF.md section 8](KPTX_PAGED_PERF.md) for the decision and
...
absolutes drift between sessions. So the recommendation has two
halves:
...
- **Enabling it explicitly is a measured 1.4–1.9× win at 8B-shaped
  decode on a GB10**, reproduced in 6/6 sessions
  ([KPTX_PAGED_PERF.md](KPTX_PAGED_PERF.md)), with no session's ratio
...
`PAGED_ATTENTION`**, which is not implemented.
...
takes the platform as a parameter. On a Cloud TPU VM the whole of section 3 is
the same commands with `platform="tpu"`, once a TPU session turns
`PjrtTpuSmokeTest`'s self-skips into passes — the step-by-step is
[TPU_BRINGUP.md](TPU_BRINGUP.md), and the ordered queue is
[TPU_READINESS_AUDIT.md](TPU_READINESS_AUDIT.md) section 5.

What a TPU session owes this document, specifically: re-run section 3 on the
`tpu` platform and record **its** floor, because section 3's two floors are
...
| a Python edit changes nothing | `:maestro:jvmTest` does not declare `harness/python/**` as inputs |
| `CUDA_ERROR_INVALID_PTX` naming nothing | a non-ASCII byte in a PTX comment |
...
| the machine reboots under XLA | never create a PJRT client without `create_options` |


---

## 10. Serve a REAL Llama (CERTIFIED, this is the demo)
...
Prerequisites: the two venvs of section 0 and section 4, a CUDA GPU, JDK 25, and ~7 GB
...
the frozen oracle venv (section 0.1).

**Where vLLM sits in this:** it runs. `LLM.generate()`
against this artifact produces the same six token ids the driver below
produces — see **section 11**, which is one command. The demo below is still the
DRIVER path, and it is still the one to run first, because it is the one
that shows the runtime with no framework in the process; section 11 then shows the
...
`-PnumLayers=2` gives the cheap lane — the exact reduced model
certified against transformers, the same code path with one integer changed.

What lands beside `bodies/` and `programs/`:
...
layout by a host-side pass. The manifest's `weights.table` names
...
`dot_general` policy is TF32 (section 3's 1e-3 CUDA floor is the same fact) and the
...
* **`vllm serve`** — the HTTP server has never been started here. The
  ENGINE under it runs: `LLM.generate()` is certified in section 11.
...
* **bf16 weight tables** — the writer refuses a non-f32 staged slot by name.
  Halving the artifact needs the GRAPH to be bf16.
...
## 11. Serve it through vLLM (CERTIFIED)

The same artifact section 10 built, answering through **vLLM 0.29.0's own
...
Prerequisite: section 10.2 has run and `/tmp/tl-llama` exists.
...
Identical to section 10.3's `generatedTokens` and to the transformers oracle's.
