...
object Tensors {
    ...
    /**
     * Rank-3 tensor constructor. Mirrors [f32Matrix] for one axis higher.
    ...
    /**
     * Rank-4 tensor constructor, mirroring [f32Tensor3] for the NCHW
     * conv/pool tensors and their OIHW/IOHW kernels. Callers brand the axes as they do for the lower ranks —
     * typically all `Sym`, since the conv spatial extents are runtime facts.
     */
    /**
     * Rank-1 I32 tensor constructor: the index vectors `embedding`
    ...
    /**
     * Rank-2 I32 tensor constructor: the batched `[B, N]` index
    ...
    /**
     * bf16 scalar constructor. Takes an f32 value and
    ...
    /** [f32Vector]'s bf16 twin: narrows each element (RNE). */
    ...
    /** [f32Matrix]'s bf16 twin: narrows each element (RNE). */
...
/** [hostF32]'s I32 twin, for `embedding`'s index vectors. */
